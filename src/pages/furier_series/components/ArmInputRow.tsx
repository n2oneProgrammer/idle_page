    ...
    return (
        <Grid container spacing={2} style={{marginTop: 10}}>
            <Grid item xs={4}><TextField type={"number"} defaultValue={data.angle} label={"Start Angle"}
                                         onChange={(v) => onChange(Object.assign(data, {angle: parseFloat(v.target.value) || 0}))}/></Grid>
            <Grid item xs={4}><TextField type={"number"} defaultValue={data.speed} label={"Speed"}
                                         onChange={(v) => onChange(Object.assign(data, {speed: parseFloat(v.target.value) || 0}))}/></Grid>
            <Grid item xs={3}><TextField type={"number"} defaultValue={data.radius} label={"radius"}
                                         onChange={(v) => onChange(Object.assign(data, {radius: parseFloat(v.target.value) || 0}))}/></Grid>
